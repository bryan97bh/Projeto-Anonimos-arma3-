...
else
{
	...
	if(_tempVar) exitwith {
		hint format["Este objeto pertenca a %1 e eles estão por perto, você não pode suportar isso.", _objet getVariable "R3F_Side"]; R3F_LOG_mutex_local_verrou = false;
	};
	...
	if (((_objet distance getMarkerPos "_BluBaseMarker") < 100) && !(side player == blufor)) exitwith {
		hint "Esta base só pode ser alterada por Blufor"; R3F_LOG_mutex_local_verrou = false;
	};
	
	if (((_objet distance getMarkerPos "_OPFBaseMarker") < 100) && !(side player == opfor)) exitwith {
		hint "Esta base só pode ser alterada por Opfor"; R3F_LOG_mutex_local_verrou = false;
	};
			...
			if (count crew _objet == 0 || getNumber (configFile >> "CfgVehicles" >> (typeOf _objet) >> "isUav") == 1) then
			{
				...
				if (_objet getVariable "R3F_LOG_est_deplace_par" == _joueur && isNull (_objet getVariable ["R3F_LOG_est_transporte_par", objNull])) then
				{
					...
					while {!isNull R3F_LOG_joueur_deplace_objet && _objet getVariable "R3F_LOG_est_deplace_par" == _joueur && alive _joueur} do
					{
						...
						if (_in_rad_2 && !(_objet isKindOf "ReammoBox_F") && !(_objet iskindof "StaticWeapon") && !(_objet iskindof "Land_TripodScreen_01_dual_v1_F") ) then { 		
							//warning range
							systemChat "Se você carregar um item perto de uma base travada, o item será solto e travado.";
				...
				//Anônimos - Allow r3f for all objects which where moveable before - should fix apoc airdrop problem
