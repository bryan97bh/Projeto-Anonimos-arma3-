...
FAR_Eject_Injured =
{
	...
	{
		//AJ-EDIT 16.11.2017 /Staynex Force move out for ALL units - not only friendly units #staticgunproblem
		if (UNCONSCIOUS(_x)) then
		{
			moveOut _x;
			unassignVehicle _x;
			_x setVariable ["FAR_cancelAutoEject", false, true];
		};
	} forEach crew _veh;
}
call mf_compile;
...
FAR_Check_Revive =
{
    ...
    // show Revive for everybody
    IS_MEDIC(HEALER) && FAR_Check_Injured_Action
}
call mf_compile;
...
FAR_Check_Load_Dragged =
{
    ...
    //AJ Edit 26.09.2017 Show Load In AddAction also for enemies /Staynex
...
FAR_Check_Eject_Injured =
{
    ...
    //AJ-EDIT 26.09.2017 Show Eject Injured Unit form Vehicle AddAction also for enemies /Staynex
