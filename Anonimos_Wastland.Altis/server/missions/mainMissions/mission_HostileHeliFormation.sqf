...
//	@file Author: JoSchaap, AgentRev, Staynex
...
_setupObjects =
{
	...
	[
        ...
        ["B_Heli_Attack_01_dynamicLoadout_F","B_Heli_Transport_03_F"],
		["O_Heli_Attack_02_dynamicLoadout_F", ["O_Heli_Light_02_dynamicLoadout_F", "orcaDAR"]],
		["I_Heli_light_03_unarmed_F", "I_Heli_light_03_dynamicLoadout_F"],
        ["B_T_VTOL_01_armed_F", "B_T_VTOL_01_infantry_F"]
	];

    // Staynex wants a formation with at least 1 Transport Heli /staynex - the following code reset the transport one with a attack heli
	// if (missionDifficultyHard) then
	// {
		// (_heliChoices select 0) set [0, "B_Heli_Light_01_armed_F"];
		// (_heliChoices select 1) set [0, "O_Heli_Light_02_v2_F"];
		// (_heliChoices select 2) set [0, "I_Heli_light_03_F"];
	// };
...
_successExec =
{
	...
	[_box1, ["US"] call BIS_fnc_selectRandom] call fn_refillbox;
	...
	[_box3, ["MILITIA", "OTHER"] call BIS_fnc_selectRandom] call fn_refillbox;
	
	{ _x setVariable ["R3F_LOG_disabled", false, true] } forEach [_box1, _box2, _box3];
	
	_successHintMessage = "The sky is clear again, the enemy heli patrol was taken out! Ammo crates have fallen near the wreck.";
};
