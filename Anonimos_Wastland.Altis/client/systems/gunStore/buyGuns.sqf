...
storePurchaseHandle = _this spawn
{
	...
	_showInsufficientFundsError =
	{
		...
		hint parseText format ["Você não tem dinheiro suficiente para<br/>""%1""", _itemText];
	...
	{
		_itemText = _this select 0;
		hint parseText format ["Você não tem espaço suficiente para<br/>""%1""", _itemText];
		playSound "FD_CP_Not_Clear_F";
		_price = -1;
	};
	...
	{
		_itemText = _this select 0;
		hint parseText format ["<t color='#ffff00'>Algum erro desconhecido ocorreu.</t><br/>A compra de  ""%1"" foi cancelado.", _itemText];
		playSound "FD_CP_Not_Clear_F";
		_price = -1;
	};
	...
	{
		_itemText = _this select 0;
		hint format ["""%1"" foi gerado do lado de fora, em frente à loja.", _itemText];
	...
	{
		_itemText = _this select 0;
		hint format ["Seu inventário está cheio ou você já tem uma arma desse tipo. Desequipe-o antes de comprar ""%1""", _itemText];
		playSound "FD_CP_Not_Clear_F";
		_price = -1;
	};
	...
	if (!isNil "_price" && {_price > -1}) then
	{
		...
		if (_successHint) then { hint "Compra com sucesso" };
		playSound "FD_Finish_F";
	};
