...
//	Modified AJ Wiking - Thermals in Vehicle switches
//	Modified AJ Staynex - Arma Sling Loading switches

if (!isServer) exitWith {};

_disablevehicleThermals = ["A3W_disablevehicleThermals"] call isConfigOn;
_enableTILimitations = ["A3W_enableTILimitations"] call isConfigOn;
_disableArmaSlingLoading = ["A3W_disableArmaSlingLoading"] call isConfigOn;

// if TI Master Switch is set to 0 - allow TI for vehicles
if !(_enableTILimitations) then
{
    _disablevehicleThermals = 0;
};
...
if (_disablevehicleThermals) then
{
    //TI Disallowed
...
switch (true) do
{
    ...
    case (_class isKindOf "C_IDAP_Truck_02_water_F"):
	{
		if (_brandNew) then
		{
			_vehicle setVariable ["water",200, true];
        };
	};
